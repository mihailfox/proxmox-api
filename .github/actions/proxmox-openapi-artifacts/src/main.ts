...
import { runAutomationPipeline, type AutomationPipelineRunOptions } from "@proxmox-api/automation";
...
function coerceBoolean(input: string | undefined, defaultValue: boolean): boolean {
  ...
  core.warning(
    `Boolean input value "${input}" is not recognized. Falling back to ${String(defaultValue)}.`
  );
  return defaultValue;
}
...
async function run(): Promise<void> {
  try {
    ...
    const shouldInstallPlaywright = coerceBoolean(
      core.getInput("install-playwright-browsers") || undefined,
      true
    );
    ...
    const summaryPath =
      reportPathInput ??
      path.join(await fs.mkdtemp(path.join(os.tmpdir(), "proxmox-action-")), "summary.json");
    if (reportPathInput) {
    ...
    if (shouldInstallPlaywright) {
      ...
      const playwrightResult = await getExecOutput(
        "npx",
      ...
      if (playwrightResult.exitCode !== 0) {
        ...
        throw new Error(
          `Playwright installation failed with exit code ${playwrightResult.exitCode}.`
        );
      }
    ...
    if (extraArgs) {
      core.warning(
        "The `extra-cli-args` input is not supported in the TypeScript action and will be ignored."
      );
    }
