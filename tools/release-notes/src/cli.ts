...
function discoverChangedChangelogFiles(workspace: string, baseRef?: string, headRef = 'HEAD'): string[] {
  const resolvedHead = headRef || 'HEAD';
  let resolvedBase = baseRef;
  if (!resolvedBase || resolvedBase.length === 0) {
    const tagOutput = execSync("git tag --list 'v[0-9]*.[0-9]*.[0-9A-Za-z.+-]*' --sort=-version:refname | head -n1", {
      cwd: workspace,
      encoding: 'utf8'
    }).trim();
    if (tagOutput.length > 0) {
      ...
      const initialCommit = execSync('git rev-list --max-parents=0 HEAD', { cwd: workspace, encoding: 'utf8' })
        .split(/\r?\n/)
        .filter(Boolean)
        .at(0);
...
export function runCli(argv: string[]): void {
  ...
  process.stdout.write(`Collected ${entries.length} changelog entr${entries.length === 1 ? 'y' : 'ies'}\n`);
