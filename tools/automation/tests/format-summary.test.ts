...
function createSummary(
  overrides: Partial<AutomationPipelineResult> = {}
): AutomationPipelineResult {
  return {
    rawSnapshotPath: path.join(repoRoot, "tools/api-scraper/data/raw/proxmox-api-schema.json"),
    normalizedDocumentPath: path.join(repoRoot, "tools/api-normalizer/data/ir/proxmox-api-ir.json"),
...
describe("formatAutomationSummary", () => {
  it("renders a Markdown section with relative paths and cache status", () => {
    ...
    expect(markdown).toContain(
      "| Raw snapshot | `tools/api-scraper/data/raw/proxmox-api-schema.json` |"
    );
